  ...
  return (
    <div className="space-y-10">
      <section className="rounded-3xl border border-[#f6b2c5]/70 bg-white/90 p-8 shadow-sm">
        <h2 className="text-2xl font-semibold text-[#40111f]">Operational snapshot</h2>
        <p className="mt-2 text-sm text-neutral-600">
          Monitor catalogue health, revenue, and artisan participation across the Janakpur Art and Craft network.
        </p>
        <div className="mt-6 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          <div className="rounded-2xl border border-[#f6b2c5]/60 bg-[#ffe1ef]/60 p-5">
            <p className="text-xs font-semibold uppercase tracking-[0.35em] text-[#b03d5e]">Catalogue</p>
            <p className="mt-2 text-3xl font-semibold text-[#40111f]">{overview.productCount}</p>
            <p className="text-xs text-neutral-600">
              {overview.publishedCount} published • {overview.productCount - overview.publishedCount} in draft
            </p>
          </div>
          <div className="rounded-2xl border border-[#f6b2c5]/60 bg-white p-5">
            <p className="text-xs font-semibold uppercase tracking-[0.35em] text-[#b03d5e]">Orders</p>
            <p className="mt-2 text-3xl font-semibold text-[#40111f]">{overview.orders.total}</p>
            <p className="text-xs text-neutral-600">
              {overview.orders.active} active • Revenue {formatCurrencyFromCents(overview.orders.revenueCents)}
            </p>
          </div>
          <div className="rounded-2xl border border-[#f6b2c5]/60 bg-white p-5">
            <p className="text-xs font-semibold uppercase tracking-[0.35em] text-[#b03d5e]">Artisans</p>
            <p className="mt-2 text-3xl font-semibold text-[#40111f]">{overview.artisans}</p>
            <p className="text-xs text-neutral-600">Active collectives across Janakpur and Kathmandu.</p>
          </div>
          <div className="rounded-2xl border border-[#f6b2c5]/60 bg-white p-5">
            <p className="text-xs font-semibold uppercase tracking-[0.35em] text-[#b03d5e]">Import jobs</p>
            <p className="mt-2 text-3xl font-semibold text-[#40111f]">{overview.pendingImports}</p>
      ...
      <section className="rounded-3xl border border-[#f6b2c5]/70 bg-white/90 p-8 shadow-sm">
        <h3 className="text-xl font-semibold text-[#40111f]">Next actions</h3>
