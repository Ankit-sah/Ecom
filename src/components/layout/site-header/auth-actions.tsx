...
export function AuthActions({ session, variant = "default" }: AuthActionsProps) {
  ...
  if (session?.user) {
    ...
    if (isMinimal) {
      return (
        <div className="flex items-center gap-2 rounded-full border border-orange-500/30 bg-white/90 px-3 py-1.5 text-xs font-semibold text-orange-500 shadow-sm">
    ...
    return (
      <div className="flex items-center gap-3 rounded-full border border-orange-500/30 bg-white/85 px-3 py-2 text-sm font-medium text-orange-500 shadow-sm backdrop-blur">
        ...
        <button
          type="button"
          onClick={() => signOut({ callbackUrl: "/" })}
          className="rounded-full border border-orange-500/40 px-3 py-1 text-xs font-semibold text-orange-500 transition hover:border-orange-500 hover:text-orange-600"
        >
          Sign out
        </button>
      </div>
